...
function domElement2array(DomElement $xmlObject) {
	...
	$out = Array();
	$out[$xmlObject->nodeName] = utf8_decode($xmlObject->nodeValue);

	foreach($xmlObject->attributes as $oItem) {
		$out[$oItem->nodeName] = utf8_decode($oItem->nodeValue);
	}	
	foreach ($xmlObject->childNodes as $i => $filhos) {
		if ($filhos instanceof DOMElement) {                
	$sCurrentNodeName = utf8_decode(getCurrentNodeName($out, $filhos->nodeName));
		 ...
		 if($x != 0) {
			...
			foreach($oRes as $y => $value) {
				if(!(strpos($y, 'tr') === false)) {
					...
					echo $value['td']['td'] . ' - ' . $value['td1']['td'] . ' - ' . $value['td2']['td'] . ' - ' . $value['td3']['td'] . ' - ' . @$value['td4']['a']['href'] . ' | '.@$value['td4']['a']['a'].'<br>';
				}
			}
		 }
	 }
