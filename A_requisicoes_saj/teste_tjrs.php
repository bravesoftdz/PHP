...
if(isset($_POST['captcha'])){		 
	...
	foreach($_SESSION['sessao_php'][0] as $aSetCookie){
 	 ...
 	 $sUrl = 'http://www.tjrs.jus.br/site_php/consulta/consulta_movimentos.php?entrancia=1&comarca=santa_rosa&num_processo=10300086007&code='.$_POST['captcha'];	
	 $ch = curl_init();
	 ...
	 curl_setopt($ch, CURLOPT_COOKIE, implode('; ', $aCookiesTeste));	 
	 curl_setopt($ch, CURLOPT_FOLLOWLOCATION, true);
	 $response = curl_exec($ch);  
	 print_r($aCookiesTeste); 
	 echo $response;
} else {
 $ch = curl_init();
