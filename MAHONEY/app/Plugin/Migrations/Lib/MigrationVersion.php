...
class MigrationVersion {
    ...
    public function getMigration($name, $class, $type, $options = array()) {
        if (!class_exists($class) && (!$this->_loadFile($name, $type) || !class_exists($class))) {
            throw new MigrationVersionException(sprintf(
                    __d('migrations', 'Class `%1$s` not found on file `%2$s` for %3$s.'), $class, $name . '.php', (($type === 'app') ? 'Application' : Inflector::camelize($type) . ' Plugin')
            ));
        }
    ...
    protected function _loadFile($name, $type) {
        ...
        if (!file_exists($path . $name . '.php')) {
            throw new MigrationVersionException(sprintf(
                    __d('migrations', 'File `%1$s` not found in the %2$s.'), $name . '.php', (($type === 'app') ? 'Application' : Inflector::camelize($type) . ' Plugin')
            ));
        }
